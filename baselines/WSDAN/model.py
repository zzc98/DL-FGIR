...
def inception_v3_bap(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_dict = torch.load(config.inception_path)
...
def WSDAN():
    m = inception_v3_bap(pretrained=True, aux_logits=False, num_parts=config.parts)
    in_features = m.fc_new.in_features
    new_linear = torch.nn.Linear(in_features=in_features, out_features=config.num_classes)
    m.fc_new = new_linear
    return m
