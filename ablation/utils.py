...
def select_reconstruction_image2(x, window, n):
    batch_size, _, H, W = x.size()
    _, h, w = window.size()
    assert n <= h and n <= w, 'n is too large for window'
    window = window.view(batch_size, h * w)
    ...
    patch_size_raw = (H // h, W // w)
    patch_size_new = (H // n, W // n)
    image_new = torch.zeros((patch_size_raw[0] * n, patch_size_raw[1] * n)).cuda()
    for k in range(n ** 2):
        raw_i, raw_j = indices[:, k] / h, indices[:, k] % w
        for b in range(batch_size):
            H_start, H_end, W_start, W_end = \
                ...
                raw_j[b].item() * patch_size_raw[1], \
                raw_j[b].item() * patch_size_raw[1] + patch_size_raw[1]
            image_new[:, :, H_start:H_end, W_start:W_end] = x[:, :, H_start:H_end, W_start:W_end]
    image_new = F.interpolate(image_new, size=(patch_size_new[0] * n, patch_size_new[1] * n), mode='bilinear',
                              align_corners=True)
    return image_new
